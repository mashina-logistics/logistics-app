        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 15 }}>
            <div>
              ...
              <input type="text" name="sender" value={formData.sender} onChange={handleInputChange} required style={{ width: '100%', padding: 8 }} placeholder="Название компании" />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: 5, fontWeight: 'bold' }}>Получатель *</label>
              <input type="text" name="receiver" value={formData.receiver} onChange={handleInputChange} required style={{ width: '100%', padding: 8 }} placeholder="Название компании" />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: 5, fontWeight: 'bold' }}>Плательщик *</label>
              <input type="text" name="payer" value={formData.payer} onChange={handleInputChange} required style={{ width: '100%', padding: 8 }} placeholder="Кто оплачивает" />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: 5, fontWeight: 'bold' }}>Город доставки *</label>
              <input type="text" name="delivery_city" value={formData.delivery_city} onChange={handleInputChange} required style={{ width: '100%', padding: 8 }} placeholder="Город" />
            </div>
          </div>
            ...
            <div key={index} style={{ border: '1px solid #ddd', padding: 15, marginBottom: 10, borderRadius: 4, background: '#fafafa' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                ...
                {formData.waypoints.length > 2 && (
                  <button type="button" onClick={() => removeWaypoint(index)} style={{ color: 'red', background: 'none', border: 'none', cursor: 'pointer' }}>
                    Удалить
                  </button>
                )}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  ...
                  <select value={wp.waypoint_type} onChange={(e) => handleWaypointChange(index, 'waypoint_type', e.target.value)} style={{ width: '100%', padding: 8 }}>
                ...
                <div>
                  ...
                  <input type="text" value={wp.city} onChange={(e) => handleWaypointChange(index, 'city', e.target.value)} style={{ width: '100%', padding: 8 }} placeholder="Город" />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ display: 'block', marginBottom: 5 }}>Адрес *</label>
                  <input type="text" value={wp.address} onChange={(e) => handleWaypointChange(index, 'address', e.target.value)} required style={{ width: '100%', padding: 8 }} placeholder="Улица, дом" />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 5 }}>Контактное лицо</label>
                  <input type="text" value={wp.contact_name} onChange={(e) => handleWaypointChange(index, 'contact_name', e.target.value)} style={{ width: '100%', padding: 8 }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 5 }}>Телефон</label>
                  <input type="text" value={wp.contact_phone} onChange={(e) => handleWaypointChange(index, 'contact_phone', e.target.value)} style={{ width: '100%', padding: 8 }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 5 }}>Паллеты</label>
                  <input type="number" value={wp.pallets} onChange={(e) => handleWaypointChange(index, 'pallets', e.target.value)} style={{ width: '100%', padding: 8 }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 5 }}>Вес (кг)</label>
                  <input type="number" value={wp.weight_kg} onChange={(e) => handleWaypointChange(index, 'weight_kg', e.target.value)} style={{ width: '100%', padding: 8 }} />
                </div>
              </div>
            </div>
          ))}

          <button type="button" onClick={addWaypoint} style={{ marginBottom: 15, padding: '8px 16px', background: '#f0f0f0', border: '1px solid #ccc', borderRadius: 4, cursor: 'pointer' }}>
            + Добавить точку маршрута
          </button>

          <button type="submit" disabled={loading || !formData.driver_id} style={{ width: '100%', padding: 12, background: loading ? '#ccc' : '#4CAF50', color: 'white', border: 'none', borderRadius: 4, fontSize: 16, cursor: loading ? 'not-allowed' : 'pointer' }}>
